...
def ar1_cov(sds, rho=0.6):
    """根据标准差序列生成AR(1)协方差矩阵"""
    n = len(sds)
    cov = np.zeros((n, n))
    for i in range(n):
        for j in range(n):
            cov[i, j] = sds[i] * sds[j] * (rho ** abs(i - j))
    return cov


def draw_correlated(n, means, sds, rho=0.6):
    """为n个患者生成跨4个时点相关的指标值"""
    cov = ar1_cov(sds, rho)
    vals = np.random.multivariate_normal(means, cov, size=n)
    return vals
...
def generate_latent_trajectories(group_code, n=200000):
    """基于患者级潜变量 u 和 r 生成跨指标协调的纵向相关轨迹"""
    severity = np.random.normal(0, 1, size=n)   # 越高 = 病情越重/功能越差
    if group_code in ("G1", "G2"):
        recovery = np.random.normal(0, 1, size=n)  # 越高 = 恢复越快
    else:
        recovery = np.zeros(n)

    alpha = 0.75
    beta = 0.25
    gamma = np.sqrt(1 - alpha**2 - beta**2)  # ~0.661

    # 预计算 T0 均值用于 delta 符号
    base_means = {m: {tp: TARGETS[m][tp][group_code][0] for tp in TIMEPOINTS} for m in METRICS}

    # 为每个指标生成时间噪声 (AR1, rho=0.6)
    temporal = {}
    for m in METRICS:
        sds = [TARGETS[m][tp][group_code][1] * gamma for tp in TIMEPOINTS]
        temporal[m] = draw_correlated(n, means=[0.0, 0.0, 0.0, 0.0], sds=sds, rho=0.6)
    ...
    for i in range(n):
        ...
        for tp_idx, tp in enumerate(TIMEPOINTS):
            vals = {}
            for m in METRICS:
                mu = TARGETS[m][tp][group_code][0]
                sd = TARGETS[m][tp][group_code][1]
                delta = mu - base_means[m]["T0"]
                direction = np.sign(delta) if delta != 0 else 0.0
                # recovery >0 时朝改善方向偏移
                rec = recovery[i] * sd * beta * direction
                # severity 的符号：FMA/BBS/ADL 高分=好，故severity增加时这些值应减少；
                # TUGT/MAS/CSS 高分=差，severity增加时这些值应增加
                sev_effect = -severity[i] * sd * alpha if m in ("FMA_LE", "BBS", "ADL") else severity[i] * sd * alpha
                raw = mu + sev_effect + rec + temporal[m][i, tp_idx]
                vals[m] = raw

            # 取整与截断
            traj[tp] = {
                "FMA_LE": int(np.clip(round(vals["FMA_LE"]), 0, 34)),
                "BBS": int(np.clip(round(vals["BBS"]), 0, 56)),
                "TUGT_raw": round(vals["TUGT"], 1),
                "MAS": float(discretize_mas_col(np.array([vals["MAS"]]))[0]),
                "CSS": int(discretize_css_col(np.array([vals["CSS"]]))[0]),
                "ADL": int(np.clip(round(vals["ADL"]), 0, 100)),
            }
        patients.append(traj)
    return patients


def smooth_roller_coaster(proc, group_code):
    """对康复组进行小幅单调性修正，抑制过山车（阈值已放宽）"""
    if group_code in ("G3", "G4"):
        return proc

    proc = {tp: dict(proc[tp]) for tp in TIMEPOINTS}
    # FMA: 允许小波动，仅修正大跌
    for i in range(1, 3):
        prev = TIMEPOINTS[i]
        curr = TIMEPOINTS[i + 1]
        if proc[curr]["FMA_LE"] < proc[prev]["FMA_LE"] - 5:
            proc[curr]["FMA_LE"] = proc[prev]["FMA_LE"] - np.random.randint(0, 4)
    # BBS
    for i in range(1, 3):
        prev = TIMEPOINTS[i]
        curr = TIMEPOINTS[i + 1]
        if proc[curr]["BBS"] < proc[prev]["BBS"] - 6:
            proc[curr]["BBS"] = proc[prev]["BBS"] - np.random.randint(0, 5)
    # ADL: latent 已保证高度相关，不做额外修正以避免方差压缩
    return proc
...
def subset_loss(subset, group_code):
    ...
    for metric in METRICS:
        for tp in TIMEPOINTS:
            ...
            sd_weight = 0.3 if metric == "TUGT" else 0.45
            mean_weight = 1 - sd_weight
            sd_err += abs(actual_sd - sigma) / max(sigma, 0.1) * sd_weight
            mean_err += abs(np.mean(vals) - mu) / max(mu, 0.1) * mean_weight
    return (mean_err + sd_err) / n
...
def generate_group(group_code, n_pool=200000, n_target=30, n_search=80000):
    print(f"处理 {group_code} ...")
    base_patients = generate_latent_trajectories(group_code, n=n_pool)

    hard_ok_pool = []
    for base in base_patients:
        smoothed = smooth_roller_coaster(base, group_code)
        proc = preprocess_traj(smoothed, group_code)
...
def main():
    gp = {}
    for gc in GROUPS:
        gp[gc] = generate_group(gc, n_pool=200000, n_target=30, n_search=150000)
    ...
    out = "/Users/leyixu/Ai cowork/research/stroke_simulated_120_v7.xlsx"
