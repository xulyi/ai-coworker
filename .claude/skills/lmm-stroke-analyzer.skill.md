...
description: 脑卒中康复纵向数据 LMM 分析与目标达成检查（R 版本）。一键读取 CSV/Excel，输出 T0 基线检验、所有参数所有时间点均值±标准差、LMM / 非参数 pairwise 目标达成总表（含理想/偏强/不显著状态标记），并导出格式化 Excel。
type: prompt
metadata:
  version: "1.0.0"
  author: "乐义"
...
| 脚本依赖缺失 | 先在 R 中 `install.packages()` 缺失包再重跑 |
